      ...
      if (val.hasOwnProperty("src") && val.hasOwnProperty("dist")) {
        if (["custom", "media", "api", "misc"].indexOf(key) !== -1) {
            ...
            if (val.src.styles[0].indexOf("style.scss") !== -1) {
              presets.forEach((preset) => {
                let buildStylePresetTask = (cb) => {
                  val.src.styles[0] =
                    "{$config.path.src}/sass/presets/" + preset + "/style.scss";
                  val.dist.styles =
                    "{$config.dist}/css/style." + preset + ".bundle.css";
                  bundle(val);
                  cb();
                };
          ...
          if (
            ...
            val.src.override.styles[0].indexOf("plugins.scss") !== -1
          ) {
            presets.forEach((preset) => {
